...
import { Eye, EyeOff, Lock, Mail, User, Phone, ArrowLeft, CheckCircle2, AlertCircle, Loader2 } from 'lucide-react';
...
function SignUpContent() {
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!agreeTerms) {
      setErrorMsg('Please agree to the Terms of Service & Privacy Policy to register.');
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        setSuccessMsg(`Welcome to Googles Beach Hotel, ${fullName}! Your account has been created.`);
        setTimeout(() => {
          router.push('/');
        }, 1500);
      } else {
        setErrorMsg(data.message || 'Account registration failed.');
      }
    } catch {
  ...
  // Direct Google OAuth registration handler
  const directGoogleSignUp = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        const data = await response.json();
        if (data.success) {
          setSuccessMsg(`Account registered via Google for ${data.user.name}!`);
          setTimeout(() => {
            router.push('/');
          }, 1500);
        }
    ...
    onError: () => setErrorMsg('Google Sign-Up was cancelled.')
  });
      ...
      <div className="relative z-10 w-full max-w-md bg-[#1F1F1F]/90 backdrop-blur-xl border border-stone-800 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-stone-400 text-xs font-light">
            Join Googles Beach Hotel & Spa to unlock exclusive rates and instant booking.
          </p>
        </div>
        ...
        <div className="space-y-3">
          ...
          >
            ...
            <span>Sign up directly with Google Account</span>
          </button>
        </div>

        <div className="flex items-center space-x-2 my-4">
          <div className="flex-1 h-[1px] bg-stone-800" />
          <span className="text-[10px] uppercase text-stone-500 font-semibold tracking-widest">Or Register with Email</span>
          <div className="flex-1 h-[1px] bg-stone-800" />
        </div>
        ...
        <form onSubmit={handleEmailSignUp} className="space-y-3">
          ...
          <div className="pt-1">
            <label className="flex items-start space-x-2 text-stone-400 text-xs cursor-pointer">
              <input
                type="checkbox"
                checked={agreeTerms}
              ...
              />
              <span>I agree to Googles Beach Hotel <a href="#" className="text-amber-400 underline">Terms of Service</a> & <a href="#" className="text-amber-400 underline">Privacy Policy</a>.</span>
            </label>
          </div>
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full mt-2 bg-amber-500 hover:bg-amber-400 text-stone-950 font-bold uppercase tracking-widest text-xs p-3.5 rounded-xl transition-all cursor-pointer shadow-lg disabled:opacity-60 flex items-center justify-center gap-2"
          >
            {isLoading ? (
              <>
        ...
        <div className="text-center text-xs text-stone-400 pt-2 border-t border-stone-800">
          ...
          <Link href="/signin" className="text-amber-400 font-semibold hover:underline">
            Sign In Here
          </Link>
        </div>
